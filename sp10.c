...
static int parseCountryLine(const char* line, char* cname, char* cfile);
static int parseCityLine(const char* line, char* city, int* pop);

static int insertCountryTree(Country** root, const char* name, const char* file);
static int findCountryTree(Country* root, const char* name, Country** out);

static int insertCityListSorted(City** head, const char* name, int pop);
static int loadCitiesIntoCountry(Country* c);

static int buildTreeFromFile(const char* drzaveFile, Country** root);
static int printCityList(City* head);
static int printCityListAbove(City* head, int minPop);
static int printCountryTree(Country* root);

static int freeCityList(City* head);
static int freeCountryTree(Country* root);
